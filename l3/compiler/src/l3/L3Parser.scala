...
object L3Parser {
  ...
  private class L(indexToPosition: Int=>Position) {
    ...
    private def parseIntLit(s: String, r: Int): L3Int =
      java.lang.Long.parseLong(s, r).asInstanceOf[L3Int]
    ...
    private val integer2 = (Index ~ prefix2 ~/ (sign.? ~ digit2.rep(1)).!)
      .map { case (i, s) => Lit(IntLit(parseIntLit(s, 2)))(i) }
    private val integer10 = (Index ~ (sign.? ~ digit10 ~/ digit10.rep).!)
      .map { case (i, s) => Lit(IntLit(parseIntLit(s, 10)))(i) }
    private val integer16 = (Index ~ prefix16 ~/ (sign.? ~ digit16.rep(1)).!)
      .map { case (i, s) => Lit(IntLit(parseIntLit(s, 16)))(i) }
